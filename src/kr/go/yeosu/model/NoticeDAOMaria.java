...
import kr.go.yeosu.util.MariaDB10;

public class NoticeDAOMaria implements NoticeDAO {
	...
	public ArrayList<NoticeDTO> noticeListAll(){
		...
		try {
			con = MariaDB10.getConnection();
			pstmt = con.prepareStatement(MariaDB10.NOTICE_SELECT_ALL);
	...
	public NoticeDTO getNotice(int idx){
		...
		try {
			con = MariaDB10.getConnection();
			pstmt = con.prepareStatement(MariaDB10.NOTICE_SELECT_ONE);
			pstmt.setInt(1, idx);
			rs = pstmt.executeQuery();
			if(rs.next()){
				noti.setIdx(rs.getInt("idx"));
	...
	public void updateReadCount(int idx){
		try {
			con = MariaDB10.getConnection();
			pstmt = con.prepareStatement(MariaDB10.NOTICE_READCOUNT_UPDATE);
			pstmt.setInt(1, idx);
			pstmt.executeUpdate();
		} catch (ClassNotFoundException e) { //오라클 JDBC 클래스가 없거나 경로가 다른 경우 발생
			e.printStackTrace();
	...
	public int insertNotice(NoticeDTO noti){
		...
		try {
			con = MariaDB10.getConnection();
			pstmt = con.prepareStatement(MariaDB10.INSERT_NOTICE);
	...
	public int updateNoticePro(NoticeDTO noti) {
		...
		try {
			...
			if(noti.getFile1()==null) {
				pstmt = con.prepareStatement(MariaDB10.UPDATE_NOTICE2);
				pstmt.setString(1, noti.getTitle());
				pstmt.setString(2, noti.getContent());
				pstmt.setInt(3, noti.getIdx());
			} else {
				pstmt = con.prepareStatement(MariaDB10.UPDATE_NOTICE);
				pstmt.setString(1, noti.getTitle());
				pstmt.setString(2, noti.getContent());
	...
	public int deleteNotice(int idx) {
		...
		try {
			con = MariaDB10.getConnection();
			pstmt = con.prepareStatement(MariaDB10.DELETE_NOTICE);
			pstmt.setInt(1, idx);
